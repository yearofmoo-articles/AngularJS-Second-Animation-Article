...
function minErr(module, ErrorConstructor) {
  ...
  return function() {
    ...
    message += '\nhttp://errors.angularjs.org/1.4.0-local+sha.590ec31/' +
      (module ? module + '/' : '') + code;
 ...
 */
var version = {
  full: '1.4.0-local+sha.590ec31',    // all of these placeholder strings will be replaced by grunt's
  ...
  this.$get = [function() {
    return function(existing, toAdd, toRemove) {
      ...
      forEach(toRemove, function(value, key) {
        if (flags[key] === ADD_CLASS) {
          delete flags[key];
        } else {
          flags[key] = REMOVE_CLASS;
        }
      });
...
var $$CoreAnimateQueueProvider = [function() {
  ...
  this.$get = ['$$AnimateRunner', '$$jqLite', '$rootScope', '$$animateClassResolver',
       function($$AnimateRunner,   $$jqLite,   $rootScope,   $$animateClassResolver) {
    return {
      enabled: noop,
      on: noop,
      off: noop,
...
var $AnimateProvider = ['$provide', function($provide) {
  ...
  this.$get = ['$$animateQueue', function($$animateQueue) {
    function domInsert(element, parent, after) {
      ...
      if (after) {
        var afterNode = extractElementNode(after);
        if (afterNode && !after.parentNode && !afterNode.previousElementSibling) {
          after = null;
        }
      }
    ...
    function proxy(name) {
      return function() {
        return $$animateQueue[name].apply($$animateQueue, arguments);
      };
    }
